...
bool Button_texture::load_from_file(string path, bool is_question)
{
...
bool load_button_media()
{
    ...
    for (int i = 0; i < TOTAL_BUTTONS; i++)
    {
        ...
        if (!button_texture[i].load_from_file(path, false))
        {
    ...
    question_texture.load_from_file("image/q" + to_string(current_question) + "/question.jpg", true);
        for (int i = 0; i < 4; i++)
        {
...
void create_buttons()
{
    ...
    { // question loop
        ...
        cout << "current_question: " << current_question << endl;
